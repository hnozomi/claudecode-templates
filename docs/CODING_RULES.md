...
1. 技術スタック
   ...
   Pkg Manager pnpm
   Lint / Format / Type Check Biome（lint・format・typecheck 全て実行）
...
4.2 再レンダリング最適化
memo / useMemo / useCallback は 計測して必要と判断後に導入

Context には 高頻度更新 state を渡さない

4.3 エラーハンドリング
/app/error.tsx で全体を Error Boundary 包括

機能単位の Boundary は必要に応じ追加
...
UI テストは @testing-library/react で Role / Text 検索を推奨
